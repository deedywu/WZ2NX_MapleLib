...
namespace MapleLib.WzLib.WzProperties
{
    ...
    public class WzPngProperty : WzImageProperty
    {
            ...
            try
            {
                ...
                switch (Format)
                {
                        ...
                        : // http://forum.ragezone.com/f702/wz-png-format-decode-code-1114978/index2.html#post9053713
                    {
                        ...
                        BitmapData bmpData = bmp.LockBits(rect_, ImageLockMode.WriteOnly,
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void DecompressImage_PixelDataBgra4444(byte[] rawData, int width, int height, Bitmap bmp,
            BitmapData bmpData)
        {
        ...
        internal void CompressPng(Bitmap bmp)
        {
            ...
            if (!listWzUsed) return;
            {
                ...
                using var writer = new WzBinaryWriter(memStream, WzTool.GetIvByMapleVersion(WzMapleVersion.Gms));
                writer.Write(2);
                for (int i = 0; i < 2; i++)
                {
